...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    while (1) // メインループ
    {
        ...
        for (int a = 0; a < 360; a += 180) // 180度ずらして2つの物体を動かす
        {
            for (int i = 10; i <= 180; i += 5) // 複数の物体の円運動
            {
                int x = WIDTH / 2 + RADIUS * cos(M_PI * (degree + i + a) / 120);
                int y = HEIGHT / 2 + RADIUS * sin(M_PI * (degree + i + a) / 180);
                for (int s = 5; s < 80; s += 5) DrawCircle(x, y, s * i / 120, GetColor((180 - a) + i / 3, i / 2, a + i / 3), TRUE);
            }
        }
